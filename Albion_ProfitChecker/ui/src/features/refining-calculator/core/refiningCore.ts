...
export function pipe<T>(...fns: ReadonlyArray<(value: T) => T>): (value: T) => T {
  return (value: T) => fns.reduce((acc, fn) => fn(acc), value);
}

export function withBonus(bonusFn: BonusStep, bonuses: BonusConfig): RefiningStep {
...
export function computeStationFee(itemValue: number, usageFeePer100: number, nutritionFactor: number): number {
...
export function computeReturnRate(input: RefiningInput): number {
  const combined = input.baseReturnRate + input.bonuses.cityBonusRate + input.bonuses.refiningBonusRate;
  const withFocus = input.bonuses.focusEnabled ? combined + input.bonuses.focusReturnRate : combined;
...
export function getReturnRatePresetConfig(preset: ReturnRatePreset): {
  ...
  if (preset === "bonus_city") {
    return { baseReturnRate: 0.152, cityBonusRate: 0.215, refiningBonusRate: 0, focusEnabled: false, focusReturnRate: 0 };
  }
  if (preset === "bonus_city_focus") {
    return { baseReturnRate: 0.152, cityBonusRate: 0.215, refiningBonusRate: 0, focusEnabled: true, focusReturnRate: 0.172 };
  }
  return { baseReturnRate: 0.152, cityBonusRate: 0, refiningBonusRate: 0, focusEnabled: false, focusReturnRate: 0 };
}

export function applyBonuses(state: RefiningState, bonuses: BonusConfig): RefiningState {
  const nextReturnRate = clamp(
    state.returnRate + bonuses.cityBonusRate + bonuses.refiningBonusRate + (bonuses.focusEnabled ? bonuses.focusReturnRate : 0),
    0,
    0.99
  );
...
function finalizeProfit(state: RefiningState): RefiningState {
  ...
  const profitPercent = state.totalCost > 0 ? (profit / state.totalCost) * 100 : 0;
...
export function calculateRefining(input: RefiningInput): RefiningResult {
  ...
  const flow = pipe(computeBase, withBonus(applyBonuses, input.bonuses), applyReturnSavings, finalizeProfit);
...
export function createRefiningInput(params: BuildInputParams): RefiningInput {
  return {
    ...
    nutritionFactor: typeof params.nutritionFactor === "number" ? params.nutritionFactor : 0.1125,
...
export function makeRefiner(config: RefinerConfig) {
  return (variant: RefineVariant, tierInputs: ReadonlyArray<RefineTierInput>, usageFeePer100: number): RefiningResult =>
    calculateRefining(
      createRefiningInput({
        variant,
        tierInputs,
